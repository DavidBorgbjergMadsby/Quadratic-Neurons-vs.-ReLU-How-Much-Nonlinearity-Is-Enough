...
SWEEP_ROOT=./outputs/food101_l40s_atrous_budget_scaling_${LSB_JOBID:-manual}
mkdir -p "${SWEEP_ROOT}"
...
for SEED in "${SEEDS[@]}"; do
  ...
  run_adapter "${SEED}" quad_dw 17 all        4 1 1e-4 0.0  "${BASE_CKPT}" "${QUAD_ADAPTER_KERNEL}" "${QUAD_ADAPTER_DILATION}"
  run_adapter "${SEED}" quad_dw 18 all        4 1 3e-4 0.0  "${BASE_CKPT}" "${QUAD_ADAPTER_KERNEL}" "${QUAD_ADAPTER_DILATION}"
  run_adapter "${SEED}" quad_dw 19 all        8 1 1e-4 0.0  "${BASE_CKPT}" "${QUAD_ADAPTER_KERNEL}" "${QUAD_ADAPTER_DILATION}"
  run_adapter "${SEED}" quad_dw 20 all        8 1 3e-4 0.0  "${BASE_CKPT}" "${QUAD_ADAPTER_KERNEL}" "${QUAD_ADAPTER_DILATION}"

  # 3) Matched LoRA comparison near its previous best setting.
  run_adapter "${SEED}" lora_dw 14 all        4 1 1e-4 0.0  "${BASE_CKPT}"
  run_adapter "${SEED}" lora_dw 15 all        4 1 3e-4 0.0  "${BASE_CKPT}"
  run_adapter "${SEED}" lora_dw 16 all        8 1 1e-4 0.0  "${BASE_CKPT}"
  run_adapter "${SEED}" lora_dw 17 all        8 1 3e-4 0.0  "${BASE_CKPT}"
