...
def create_dataloaders(
    ...
    batch_size: int,
    num_workers: int,
    scratch_experiment: bool = False,
):
    """
    For ConvNeXt adapter/fine-tuning stages, keep the pretrained ConvNeXt
    transforms. For the clean from-scratch experiment, use simple dataset-native
    transforms so the result is not tied to ImageNet pretraining assumptions.
    """
    dataset_name = dataset_name.lower()

    if scratch_experiment:
        ...
        train_data, _, class_names = get_dataset_pair(
            dataset_name=dataset_name,
            root=root,
            transform=train_transform,
        )
        _, test_data, _ = get_dataset_pair(
            dataset_name=dataset_name,
            root=root,
...
def save_checkpoint(
    ...
    ConvNeXt linear_base checkpoints include classifier_state_dict because the
    adapter stages reload that classifier. Scratch CNN checkpoints can still be
    saved with --save-non-linear-models; in that case the full model_state_dict
    is stored without assuming a ConvNeXt classifier layout.
    """
    ...
    if hasattr(model, "classifier"):
        try:
            ...
            checkpoint["in_features"] = classifier.in_features
        except Exception:
            pass
...
class QuadraticConv2dScratch(nn.Module):
    """
    Train-from-scratch quadratic convolution layer:

        y = Conv(x) + scale * sum_r Wa_r(x) * Wb_r(x)

    This differs from QuadraticDWConvAdapter: nothing is frozen, and the
    quadratic term is part of the model being trained from random initialization.
    """
    def __init__(
        self,
        in_channels: int,
        ...
        nn.init.kaiming_normal_(self.wa.weight, nonlinearity="linear")
        nn.init.kaiming_normal_(self.wb.weight, nonlinearity="linear")
    ...
    def forward(self, x):
        ...
        B, CR, H, W = a.shape
        a = a.view(B, self.out_channels, self.rank, H, W)
        b = b.view(B, self.out_channels, self.rank, H, W)
        quad_out = (a * b).sum(dim=2)

        return linear_out + self.scaling * quad_out


class ScratchCNN(nn.Module):
    """
    Small controlled CNN for Petr's from-scratch comparison.

    standard_relu_scratch:
        ordinary Conv2d layers with ReLU activations.

    identity_scratch:
        same Conv2d architecture, but ReLU is replaced by Identity.

    quadratic_identity_scratch:
        same high-level architecture, but Conv2d layers are replaced by
        quadratic convolution layers and activations are Identity.
    """
    def __init__(
        self,
        num_classes: int,
        ...
        def make_conv(in_ch: int, out_ch: int, stride: int):
            if use_quadratic:
                return QuadraticConv2dScratch(
                    in_ch,
                    out_ch,
                    kernel_size=3,
                    stride=stride,
        ...
        self.net = nn.Sequential(
            make_conv(3, width, stride=1),
            act(),
            make_conv(width, width, stride=1),
            act(),
            make_conv(width, 2 * width, stride=2),
            act(),
            make_conv(2 * width, 2 * width, stride=1),
            act(),
            make_conv(2 * width, 4 * width, stride=2),
            act(),
            make_conv(4 * width, 4 * width, stride=1),
            act(),
...
def create_model(
    ...
    scratch_width: int = 64,
):
    if stage in SCRATCH_STAGES:
        if stage == "standard_relu_scratch":
            model = ScratchCNN(
                num_classes=num_classes,
                activation="relu",
                use_quadratic=False,
                quad_rank=adapter_rank,
                width=scratch_width,
                quad_alpha=1.0 if adapter_alpha is None else adapter_alpha,
            )
        elif stage == "identity_scratch":
            model = ScratchCNN(
                num_classes=num_classes,
                activation="identity",
                use_quadratic=False,
        ...
        elif stage == "quadratic_identity_scratch":
            model = ScratchCNN(
                num_classes=num_classes,
                activation="identity",
                use_quadratic=True,
            ...
            raise ValueError(f"Unknown scratch stage: {stage}")
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=64,
        help="Width multiplier for standard_relu_scratch, identity_scratch, and quadratic_identity_scratch.",
    )
    parser.add_argument(
    ...
    scratch_experiment = args.stage in SCRATCH_STAGES
    ...
    model = create_model(
        ...
        scratch_width=args.scratch_width,
    )
    ...
    elif args.stage in SCRATCH_STAGES:
        output_dir = output_dir / f"width_{args.scratch_width}"
        if args.stage == "quadratic_identity_scratch":
            output_dir = output_dir / f"rank_{args.adapter_rank}"
...
# Petr-style from-scratch architecture comparison:
# python train_LoRA_Qudratic.py --dataset cifar10 --stage standard_relu_scratch --epochs 100 --batch-size 128 --lr 1e-3 --seed 1
# python train_LoRA_Qudratic.py --dataset cifar10 --stage identity_scratch --epochs 100 --batch-size 128 --lr 1e-3 --seed 1
# python train_LoRA_Qudratic.py --dataset cifar10 --stage quadratic_identity_scratch --epochs 100 --batch-size 128 --lr 1e-3 --adapter-rank 2 --seed 1

# Legacy ConvNeXt scratch baseline. This starts from random ConvNeXt-Tiny weights and trains the full network.
