...
def main():
    ...
    parser.add_argument(
        ...
        type=float,
        default=1.0,
        help="1.0 = full ReLU, 0.0 = no ReLU",
    )
    ...
    if args.stage == "quadratic_adapter":
        ...
        output_dir = (
            output_dir / f"rank_{resolved_lora_rank}_alpha_{resolved_lora_alpha:g}"
        )

    # 🔥 ADD THIS (important for your experiment)
    # base path
    output_dir = Path(args.output_dir) / args.dataset / args.stage

    # ONLY adapters depend on gate
    if args.stage in ["quadratic_adapter", "lora_adapter"]:
        output_dir = output_dir / f"relu_gate_{args.relu_gate}"
