...
Example:
  python run_food101_local_8q_8l_2f_winfix.py

Useful local overrides:
  python run_food101_local_8q_8l_2f_winfix.py --batch-size 32 --num-workers 4
  python run_food101_local_8q_8l_2f_winfix.py --adapter-epochs 5 --full-epochs 5
  python run_food101_local_8q_8l_2f_winfix.py --dry-run

Windows note:
  By default on Windows this writes runs to ~/quad_runs to avoid WinError 206
  path-length failures from very long project folders. Override with --output-root.
"""
...
def scope_tag(scope: str) -> str:
    return "ls" if scope == "last_stage" else "all"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--output-root", type=Path, default=None, help="Where sweep outputs are stored. Default: ~/quad_runs on Windows, ./outputs elsewhere.")
    parser.add_argument("--run-stamp", type=str, default=None)
    parser.add_argument("--torch-home", type=Path, default=None, help="Torch model cache. Default: ~/quad_torch_cache on Windows, ./torch_cache elsewhere.")
    ...
    # Keep default output/cache paths short on Windows. Long OneDrive/GitHub
    # project paths often trigger WinError 206 when nested experiment folders
    # are created. You can still override these with --output-root/--torch-home.
    if args.output_root is None:
        args.output_root = (Path.home() / "quad_runs") if os.name == "nt" else Path("outputs")
    if args.torch_home is None:
        args.torch_home = (Path.home() / "quad_torch_cache") if os.name == "nt" else Path("torch_cache")
    ...
    args.sweep_root = args.output_root / f"{args.dataset}_8q8l2f_{args.run_stamp}"
...
def main() -> None:
    ...
    if os.name == "nt":
        print("Windows path-length guard: using short output/cache defaults unless overridden.")
    ...
    # 1) Linear base.
    linear_run = f"000_lin_lr{sanitize(args.linear_lr)}_wd{sanitize(args.linear_weight_decay)}"
    append_manifest(
        manifest,
        linear_run,
        "linear_base",
    ...
    for i, combo in enumerate(QUAD_COMBOS, start=1):
        ...
        htag = "h1" if combo.train_head else "h0"
        run_name = f"1{i:02d}_q_{scope_tag(combo.scope)}_r{combo.rank}_{htag}_lr{sanitize(combo.lr)}_wd{sanitize(combo.weight_decay)}"
        append_manifest(
            manifest,
            run_name,
            "quad_dw",
    ...
    for i, combo in enumerate(LORA_COMBOS, start=1):
        head = "yes" if combo.train_head else "no"
        htag = "h1" if combo.train_head else "h0"
        run_name = f"2{i:02d}_l_{scope_tag(combo.scope)}_r{combo.rank}_{htag}_lr{sanitize(combo.lr)}_wd{sanitize(combo.weight_decay)}"
        append_manifest(
            manifest,
            run_name,
            "lora_dw",
    ...
    for i, combo in enumerate(FULL_COMBOS, start=1):
        run_name = f"3{i:02d}_f_lr{sanitize(combo.lr)}_wd{sanitize(combo.weight_decay)}"
        append_manifest(
            manifest,
            run_name,
            "full_finetune",
